...
- `allow_paid_broadcast`: (Boolean) Pass True to allow up to 1000 messages per second, ignoring broadcasting limits for a fee of 0.1 Telegram Stars per message. The relevant Stars will be withdrawn from the bot's balance
- `message_effect_id`: (String) Unique identifier of the message effect to be added to the message; for private chats only
- `video_start_timestamp`: (Integer) New start timestamp for the forwarded video
...
- `allow_paid_broadcast`: (Boolean) Pass True to allow up to 1000 messages per second, ignoring broadcasting limits for a fee of 0.1 Telegram Stars per message. The relevant Stars will be withdrawn from the bot's balance
- `message_effect_id`: (String) Unique identifier of the message effect to be added to the message; for private chats only
- `video_start_timestamp`: (Integer) New start timestamp for the copied video
- `reply_parameters`: (ReplyParameters) Description of the message to reply to
- `reply_markup`: (InlineKeyboardMarkup or ReplyKeyboardMarkup or ReplyKeyboardRemove or ForceReply) Additional interface options. A JSON-serialized object for an inline keyboard, custom reply keyboard, instructions to remove reply keyboard or to force a reply from the user.
...
# Optional arguments
- `message_thread_id`: (Integer) Unique identifier for the target message thread (topic) of the forum; for forum supergroups only
- `provider_token`: (String) Payment provider token, obtained via @BotFather. Pass an empty string to create a link for payments in [Telegram Stars](https://t.me/BotNews/90).
...
- `chat_id`: (Integer) Required if inline_message_id is not specified. Unique identifier for the target chat
- `message_id`: (Integer) Required if inline_message_id is not specified. Identifier of the sent message
- `inline_message_id`: (String) Required if inline_message_id is not specified. Identifier of the inline message
...
Use this method to refund a successful payment in [Telegram Stars](https://t.me/BotNews/90). Returns True on success.
...
Use this method to get the list of all Telegram Star transactions for the bot. Returns a [StarTransactions](https://core.telegram.org/bots/api#startransactions) object on success.
...
- `subscription_period`: (Integer) The number of seconds the subscription will be active for before the next payment. Currently, it must always be 2592000 (30 days).
- `subscription_price`: (Integer) The amount of Telegram Stars a user must pay initially and for each subsequent subscription period to be a member of the chat; 1-25000
...
Use this method to set the emoji status for a user that previously allowed the bot to manage their emoji status via the [WebApp](https://core.telegram.org/bots/webapps) method [requestEmojiStatusAccess](https://core.telegram.org/bots/webapps#requestemojistatusaccess). Returns True on success.
...
Use this method to verify a user [on behalf of an organization](https://telegram.org/verify#third-party-verification) which is represented by the bot. Returns True on success.
...
Use this method to verify a chat [on behalf of an organization](https://telegram.org/verify#third-party-verification) which is represented by the bot. Returns True on success.
...
Use this method to remove verification from a user who is currently verified [on behalf of an organization](https://telegram.org/verify#third-party-verification) represented by the bot. Returns True on success.
...
Use this method to remove verification from a chat that is currently verified [on behalf of an organization](https://telegram.org/verify#third-party-verification) represented by the bot. Returns True on success.
...
Use this method to allow the bot to cancel or re-enable subscription invoices created by the bot. Returns True on success.
...
- `is_canceled`: (Boolean) Pass True to cancel the subscription; pass False to re-enable it
...
Use this method to save a prepared inline message to be shared by a user of a Mini App. Returns a [PreparedInlineMessage](https://core.telegram.org/bots/api#preparedinlinemessage) object.
...
- `allow_group_chats`: (Boolean) Pass True if the message can be sent to groups and supergroups
- `allow_channel_chats`: (Boolean) Pass True if the message can be sent to channels
...
Use this method to get the list of gifts that can be sent by the bot to users. Requires no parameters. Returns a [Gifts](https://core.telegram.org/bots/api#gifts) object.
...
Use this method to send a gift to a user. The gift can't be converted to Telegram Stars by the user. Returns True on success.

# Required arguments
- `user_id`: (Integer) Unique identifier of the target user that will receive the gift
- `gift_id`: (String) Identifier of the gift

# Optional arguments
- `chat_id`: (Integer or String) Unique identifier for the target chat or username of the target channel (in the format `@channelusername`). If chat_id is specified, the gift will be sent to the chat instead of the user.
...
- `text_parse_mode`: (String) Mode for parsing entities in the text. See [formatting options](https://core.telegram.org/bots/api#formatting-options) for more details.
- `text_entities`: (Array of MessageEntity) A JSON-serialized list of special entities that appear in the gift text. It can be specified instead of text_parse_mode.
...
- `star_count`: (Integer) Number of Telegram Stars to pay for the gift; must be 1000 for 3 months, 1500 for 6 months, and 2500 for 12 months
